# 弹幕鱼排 Python 版

弹幕鱼排（danmuFishpiPython）是一个在 Windows 桌面上显示 [fishpi](https://fishpi.cn) 聊天室弹幕的透明悬浮窗应用。它基于 PyQt6 绘制，覆盖在所有窗口之上，适合在打游戏或看视频时同步看到聊天消息。

## 主要功能

- **透明弹幕悬浮窗**：全局置顶、穿透鼠标点击，不干扰正常操作。
- **三种显示模式**：滚动弹幕（从右向左）、右侧浮窗、左下角气泡。
- **自动登录**：首次登录后账号信息会加密保存到本地，下次启动自动连接聊天室。
- **全局快捷键**：默认按 `F9` 快速打开发送框，无需切出游戏。
- **消息过滤与关注**：支持屏蔽指定用户、隐藏红包消息、特别关注提醒。
- **浅色 / 深色主题**：可在托盘菜单一键切换。
- **图片与头像**：自动加载用户头像和消息中的图片。

## 运行环境

- Windows 10 / 11（依赖 Windows DPAPI 加密密码）
- Python 3.10 或更高版本

## 开发环境搭建

1. 进入项目目录

   ```powershell
   cd D:\Dev\danmuFishpi\danmuFishpiPython
   ```

2. 创建并激活虚拟环境（可选但推荐）

   ```powershell
   python -m venv .venv
   .venv\Scripts\Activate.ps1
   ```

3. 安装依赖

   ```powershell
   pip install -r requirements.txt
   ```

4. 启动程序

   ```powershell
   python main.py
   ```

## 使用方法

### 首次登录

1. 启动程序后，屏幕不会出现明显窗口，但系统托盘会出现一条蓝色小鱼图标。
2. 右键托盘图标，选择「设置」。
3. 在设置面板输入 fishpi 账号和密码，点击登录。
4. 登录成功后，弹幕悬浮窗会自动接收聊天室消息。

### 发送消息

- 按下全局快捷键（默认 `F9`）调出底部输入框，输入内容后回车发送。
- 如果 `F9` 被其他软件占用，程序会自动尝试 `Ctrl+Shift+Enter` 等备选快捷键。
- 也可以右键托盘图标，选择「发送消息」手动打开输入框。

### 托盘菜单说明

| 菜单项 | 作用 |
| --- | --- |
| 显示 / 隐藏弹幕 | 临时开关弹幕层 |
| 发送消息 | 打开消息输入框 |
| 设置 | 打开登录和显示设置 |
| 强制置顶 | 如果弹幕被全屏游戏覆盖，点此重新置顶 |
| 切换主题 | 在浅色 / 深色主题之间切换 |
| 退出 | 关闭程序并断开聊天室连接 |

### 设置项说明

- **显示头像 / 昵称 / 图片**：控制弹幕中是否显示对应元素。
- **显示红包**：关闭后将过滤聊天室中的红包消息。
- **弹幕速度**：1 到 10，数值越大滚动越快。
- **显示区域**：可选全屏、上半屏或下半屏。
- **透明度 / 字体 / 描边**：按个人喜好调整可读性。
- **特别关注 / 屏蔽用户**：填写 fishpi 用户 ID，用英文逗号分隔。

## 打包成 EXE

项目使用 **PyInstaller** 打包成单个可执行文件，方便在没有 Python 环境的电脑上运行。

### 1. 安装 PyInstaller

在虚拟环境中执行：

```powershell
pip install pyinstaller
```

### 2. 执行打包命令

```powershell
pyinstaller --noconfirm --onefile --windowed --name "弹幕鱼排" main.py
```

参数说明：

- `--onefile`：打包成单个 `.exe` 文件。
- `--windowed`：不显示命令行窗口。
- `--name`：生成的可执行文件名称。

### 3. 减小体积（可选）

PyInstaller 默认会把 PyQt6 等依赖完整打包，单文件体积通常超过 50 MB。可以改用以下命令，让可执行文件依赖一个独立的 `_internal` 目录，体积更合理：

```powershell
pyinstaller --noconfirm --windowed --name "弹幕鱼排" main.py
```

### 4. 找到输出文件

打包完成后，产物位于：

```text
D:\Dev\danmuFishpi\danmuFishpiPython\dist\弹幕鱼排.exe
```

直接运行该文件即可。首次运行仍会提示登录，之后自动连接。

## 常见问题

- **托盘找不到图标**：图标可能被折叠在系统托盘的展开区域里。
- **全屏游戏看不到弹幕**：某些独占全屏游戏会覆盖桌面层，建议将游戏切换为「无边框窗口」模式，或使用托盘里的「强制置顶」。
- **快捷键失效**：在设置中换一个未被占用的组合键，例如 `Ctrl+Alt+M`。
- **登录失败**：检查网络能否访问 `https://fishpi.cn`，并确认账号密码正确。

## 目录结构

```text
.
├── main.py              # 程序入口，管理窗口与事件循环
├── danmu_engine.py      # 弹幕动画与轨道计算
├── overlay.py           # 透明悬浮窗绘制
├── chatroom.py          # WebSocket 聊天室连接
├── auth.py              # fishpi 登录
├── config.py            # 配置保存与 DPAPI 加密
├── hotkey.py            # 全局快捷键
├── input_box.py         # 发送消息输入框
├── settings_window.py   # 设置界面
├── tray.py              # 系统托盘
├── requirements.txt     # Python 依赖
└── README.md            # 本文件
```
