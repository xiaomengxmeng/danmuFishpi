...
from image_cache import _hash_url, _cache_path, _prune, default_cache_dir
...
def test_prune_removes_oldest_until_under_bytes(workdir):
    ...
    assert os.path.exists(os.path.join(workdir, "c.txt"))      # non-.img untouched


def test_prune_respects_file_cap(workdir):
    ...
    assert removed == 1
    assert len([f for f in os.listdir(workdir) if f.endswith(".img")]) == 2
